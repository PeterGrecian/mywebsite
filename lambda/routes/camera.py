...
# Precomputed moonrise/set intervals during the dark window (20:00-04:00 UTC),
# per night. Regenerate via `astro/bin/regen-moon-up`. Format: "HH-HH"
# (open ended: '-HH' = up from window start until HH, 'HH-' = up from HH
# until window end). 'all' = up the entire window. Missing key = moon
...
def _moon_up_str(year: int, month: int, day: int) -> str | None:
    """Return the compact moon-up interval string for this night, or None."""
    return _MOON_UP.get(f"{year:04d}-{month:02d}-{day:02d}")
...
def render_starcam_nights_index(nights, *, weeks_limit=None, hero_url=None,
    ...
    for w in range(n_weeks):
        ...
        for i in range(7):
            ...
            moon_str = _moon_up_str(d.year, d.month, d.day)
            moon_html = (f'<span style="position:absolute;bottom:3px;right:5px;'
                         f'color:#8E8E93;font-size:0.65rem;line-height:1;'
                         f'font-family:monospace;">{moon_str}</span>'
                         ) if moon_str else ''
            day_label = f'{d.day}'
            if d > today:
...
def render_starcam_night_results(night_str, summary, urls):
    ...
    moon_up = _MOON_UP.get(night_str)
    moon_str = f"Moon up {moon_up}" if moon_up else "Moon-free"
...
<div style="color:#8E8E93;font-size:0.85rem;margin-top:0.4rem;font-family:monospace;">{moon_str}</div>
<div class="stats">
